'''
Module for DataProcessLayerBase class to handle input/output processing for machine learning models.

Handles data type checking, validation, and conversion to ensure that input data is in the correct format for model training and prediction.
...
class DataProcessLayerBase:
    ...
    def _check_y(self, y, n_samples=None):
        ...
        if y_arr.ndim == 2:
            # Flatten single-column/row targets
            if y_arr.shape[1] == 1:
                y_arr = y_arr.ravel()
            elif y_arr.shape[0] == 1:
                y_arr = y_arr.ravel()
            else:
                raise ValueError(f"y must be 1D or a single column, got shape {y_arr.shape}.")
        return y_arr
