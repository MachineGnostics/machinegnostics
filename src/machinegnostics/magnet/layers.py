...
class BaseLayer(metaclass=ABCMeta):
    ...
    @abstractmethod
    def forward(self, x: typing.Union[np.ndarray, Variable]) -> Variable:
        pass
    ...
    def _process_input(self, x: typing.Union[Variable, np.ndarray]) -> np.ndarray:
    ...
    def _wrap_out(self, out) -> Variable:
        out = Variable(out)
...
class ParamLayer(BaseLayer):
    def __init__(self, w_shape, activation, w_initializer, b_initializer, use_bias):
        ...
        elif isinstance(w_initializer, init.BaseInitializer):
        ...
        if use_bias:
            ...
            elif isinstance(b_initializer, init.BaseInitializer):
