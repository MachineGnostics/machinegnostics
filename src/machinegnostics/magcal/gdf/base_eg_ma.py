...
class BaseMarginalAnalysisEGDF:
    ...
    #         d2_loss = np.abs(d2)  # Second derivative should be zero at maximum
    #         egdf_loss = np.abs(egdf_at_z0 - 0.5)  # EGDF should be 0.5
    #         ratio_loss = np.abs(d1 / d2) if np.abs(d1) > 1e-12 else 0  # Stability measure
    #         sh = egdf_extended.hi.sum()
            
    #         # Combined loss with weights
    #         loss = (d2_loss + 100 * egdf_loss + 0.5 * ratio_loss) * sh * 1e3 # NOTE can be improved in the future
    #         loss_history.append(loss)
